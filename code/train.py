...
from utils import set_seeds, _normalize, get_device, CWLoss
import os
import time
import argparse
...
def get_args():
    ...
    parser.add_argument('--batch_size', type=int,
                        default=48, help='Number of trainig samples/batch')
    ...
    parser.add_argument('--lam_spa', type=float,
                        default=0.001, help='lambda of spatial loss')
    parser.add_argument('--lam_qua', type=float,
                        default=0.0001, help='lambda of quadratic loss')
    # parser.add_argument('--lam_qua', type=float,
    #                     default=0.003, help='lambda of quadratic loss')
...
# else:
#     netG = GeneratorResnet(eps=args.eps / 255.)
# netG = GeneratorResnet()        # 用1.0默认
netG = GeneratorResnet(data_dim='high')
netG = netG.to(device)
...
for epoch in range(args.epochs):
    ...
    if epoch % 10 == 9:
        logging.info(f'epoch {epoch} was saved.')
...
# torch.save(netG.state_dict(), os.path.join(now, 'netG_{}_{}_eps{}_lam_spa{}_loss{:.3}.pth'.format(
#             args.target, args.model, args.eps, lam_spa, train_loss / len(train_loader))))
